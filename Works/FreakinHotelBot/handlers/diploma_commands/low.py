...
from states.info_states import LowCommand
...
def input_low_city(message: Message) -> None:
    """ Запрос города """
    ...
    bot.set_state(user_id=message.from_user.id,
                  state=LowCommand.input_low_city,
                  chat_id=message.chat.id)
    bot.send_message(chat_id=message.chat.id, text='🌆 Пожалуйста, введи город (en_US)')


@bot.message_handler(state=LowCommand.input_low_city)
def get_low_city(message: Message) -> None:
    """ Получение города и запрос максимальной цены отеля """
    ...
    if not message.text.isalpha():
        ...
        bot.set_state(user_id=message.from_user.id,
                      state=LowCommand.max_price,
                      chat_id=message.chat.id)
        bot.send_message(chat_id=message.chat.id, text='Well!\nТеперь введи максимально'
...
@bot.message_handler(state=LowCommand.max_price)
def get_hotel_list(message: Message) -> None:
    """ Получение максимальной цены отеля и выдача пользователю списка актуальных отелей """
    ...
    if not message.text.isdigit():
        ...
        try:
            ...
            if qty_of_list:
                ...
                bot.set_state(user_id=message.from_user.id,
                              state=LowCommand.get_low_detail,
                              chat_id=message.chat.id)
            ...
            bot.send_message(chat_id=message.chat.id, text='️📉 Время ожидания от сервера превышено, попробуй еще раз')
    ...
    try:
        ...
        try:
            ...
            if hotels['details']:
                ...
                bot.send_message(chat_id=call.message.chat.id, text='️❔ Такого ID не найдено :(')

        except TypeError:
            bot.send_message(chat_id=call.message.chat.id, text='❕ Что-то не так с ID...')

    except ReadTimeout:
        bot.send_message(chat_id=call.message.chat.id, text='️📉 Время ожидания от сервера превышено, попробуй еще раз')
